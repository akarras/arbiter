...
- Test fixture (not committed): `the local fixture replay`. Tests skip, not fail, when absent.
...
```bash
cargo run --release -- "the local fixture replay" -o "the session scratchpad\tuonela.html"
```

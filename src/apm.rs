...
/// Blizzard's 16 loops/game-second "Normal" clock times the 1.4 Faster
/// multiplier. `replay::load` enforces this by rejecting any replay whose
/// `game_speed` is not "Faster" (`GAME_SPEED_FASTER`, 4), since this constant
/// only holds at that speed.
///
/// `tests/real_replay.rs` checks `game_duration_secs` (from
/// `replay.gamemetadata.json`'s `Duration`, converted via
/// `replay::game_secs_to_real`, which also multiplies by this constant)
/// against `loops_to_secs(replay.duration_loops)` on the fixture. Because
/// both sides go through the same `* 16 / LOOPS_PER_SECOND` conversion, that
/// assertion cancels `LOOPS_PER_SECOND` algebraically and only confirms the
/// summed loop count matches the game's recorded duration in 16-loop units
/// (`duration_loops ≈ 16 × Duration`); it is insensitive to whether this
/// constant's value is actually 22.4, and does not by itself verify the
/// constant.
